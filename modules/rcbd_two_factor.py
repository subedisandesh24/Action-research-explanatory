...
def run_summary_mode(uploaded_file):
    try:
        ...
        # Scan Column 0 dynamically for structural anchors
        ...
        if any(v is None for v in [idx_A, idx_B, idx_cv, idx_interaction, idx_grand]):
            st.error("Could not find the structural anchors (Factor A, Factor B, CV, Interaction, Grand Mean) in Column A. Check table formatting.")
            return
            
        # Parse stats rows between anchors
        ...
        # Parse Factor A levels
        factor_a_levels = []
        for idx in range(idx_A + 1, idx_A_sem):
            factor_a_levels.append(str(df_raw.iloc[idx, 0]).strip())
            
        # Parse Factor B levels
        factor_b_levels = []
        for idx in range(idx_B + 1, idx_B_sem):
            factor_b_levels.append(str(df_raw.iloc[idx, 0]).strip())
            
        # Parse Factors Labels
        ...
        # Header Row contains parameters
        parameters = [str(x).strip() for x in df_raw.iloc[0].tolist()[1:] if pd.notna(x)]
        st.success(f"Detected Factors: Factor A: {factor_a_label}, Factor B: {factor_b_label}")
        
        # Display Preview Options
        group_1_name = st.text_input("First Table Title", f"Effect of {factor_a_label.lower()} and {factor_b_label.lower()} on morphological traits", key="2f_g1_name")
        group_1_cols = st.multiselect("Select parameters for Table 1", parameters, default=parameters[:len(parameters)//2], key="2f_g1_cols")
        group_2_name = st.text_input("Second Table Title", f"Effect of {factor_a_label.lower()} and {factor_b_label.lower()} on physiological and yield attributes", key="2f_g2_name")
        ...
        if st.button("Generate Two-Factor Word Report (Summary Mode)", key="2f_btn_summary"):
            doc = Document()
            doc.add_heading("Two-Factor Factorial RCBD Report Draft", 0)
            ...
            for g_title, g_cols, table_num in groups:
                ...
                for param in g_cols:
                    ...
                    # Parse F-value and Significance
                    ...
                    elif "*" in f_val_A: p_a = "(p < 0.05)"
                    
                    # Factor B Significance
                    sig_b = "significant"
                    p_b = "(p < 0.05)"
                    if "ns" in f_val_B: sig_b, p_b = "nonsignificant", "(p > 0.05)"
                    elif "**" in f_val_B: p_b = "(p < 0.01)"
                    elif "*" in f_val_B: p_b = "(p < 0.05)"
                    
                    # Interaction Significance
                    sig_interaction = "significant"
                    ...
                    elif "*" in f_val_AB: p_interaction = "(p < 0.05)"
                    
                    # Extract Means and DMRT Letters for Factor A
                    a_means = []
                    for i, lvl in enumerate(factor_a_levels):
                        raw_val = df_raw.iloc[idx_A + 1 + i, col_idx]
                        num, let = parse_dmrt_value(raw_val)
                        try: a_means.append((float(num), let, lvl))
                        except ValueError: pass
                    
                    # Sort to find Top, Par, and Bottom of Factor A
                    ...
                    # Extract Factor B Means
                    b_means = []
                    for i, lvl in enumerate(factor_b_levels):
                        raw_val = df_raw.iloc[idx_B + 1 + i, col_idx]
                        num, let = parse_dmrt_value(raw_val)
                    ...
                    if sig_b == "nonsignificant":
                        at_par_b_str = f"all application rates"
                    
                    grand_mean = str(df_raw.iloc[idx_grand, col_idx])
                    
                    # Select template based on significance matrix
                    if sig_interaction == "nonsignificant" and sig_a == "nonsignificant" and sig_b == "nonsignificant":
                        ...
                        # Fallback for significant interaction if encountered in summary
                    ...
                    desc = template.format(
                        ...
                        top_a=top_a, top_val_a=f"{top_val_a:.2f}", top_let_a=top_let_a,
                        at_par_a=at_par_a_str,
                        low_a=low_a, low_val_a=f"{low_val_a:.2f}", low_let_a=low_let_a,
                        top_b=top_b, top_val_b=f"{top_val_b:.2f}", top_let_b=top_let_b,
                        at_par_b=at_par_b_str,
                        low_b=low_b, low_val_b=f"{low_val_b:.2f}", low_let_b=low_let_b,
                        grand_mean=grand_mean,
                        combination_top="N/A", combination_low="N/A", at_par_comb="N/A"
                    )
                ...
                # Reconstruct structured APA Table
                ...
                # Helper function to append rows dynamically from raw structure
                def append_row_from_idx(target_idx, italic=False):
                ...
                # Add Factor A
                append_row_from_idx(idx_A, italic=True)
                for i in range(len(factor_a_levels)):
                    append_row_from_idx(idx_A + 1 + i)
                append_row_from_idx(idx_A_sem)
                append_row_from_idx(idx_A_f)
                append_row_from_idx(idx_A_lsd)
                
                # Add Factor B
                append_row_from_idx(idx_B, italic=True)
                for i in range(len(factor_b_levels)):
                    append_row_from_idx(idx_B + 1 + i)
                append_row_from_idx(idx_B_sem)
                append_row_from_idx(idx_B_f)
                append_row_from_idx(idx_B_lsd)
                
                # Summary rows
                append_row_from_idx(idx_cv)
                append_row_from_idx(idx_interaction)
                append_row_from_idx(idx_grand)
            ...
            st.download_button("Download Report (.docx)", data=bio, file_name="Summarized_TwoFactor_Report.docx", mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document", key="btn_d_2f_sum")
    except Exception as e:
        st.error(f"Error parsing Summarized Two-Factor Table: {e}")

# --- Raw Two-Factor Analysis Mode ---
def run_raw_mode(uploaded_file):
    try:
        ...
        factor_a_col = st.selectbox("Select Factor A Column", cols, index=0, key="raw_2f_fa")
        factor_b_col = st.selectbox("Select Factor B Column", cols, index=1, key="raw_2f_fb")
        block_col = st.selectbox("Select Block/Replication Column", cols, index=2, key="raw_2y_bk_f")
        response_cols = st.multiselect("Select Response Parameters to Analyze", cols, default=cols[3:], key="raw_2f_resp")
        ...
        if response_cols:
            ...
            group_1_name = st.text_input("First Table Title", f"Effect of {treatment_label_a} and {treatment_label_b} on growth attributes", key="raw_2f_g1_name")
            group_1_cols = st.multiselect("Select parameters for Table 1", response_cols, default=response_cols[:len(response_cols)//2], key="raw_2f_g1_cols")
            group_2_name = st.text_input("Second Table Title", f"Effect of {treatment_label_a} and {treatment_label_b} on yield attributes", key="raw_2f_g2_name")
            group_2_cols = st.multiselect("Select parameters for Table 2", [c for c in response_cols if c not in group_1_cols], default=[c for c in response_cols if c not in group_1_cols], key="raw_2f_g2_cols")
            
            if st.button("Generate Two-Factor Word Report from Raw Data", key="btn_raw_2f_calc"):
                doc = Document()
                doc.add_heading("Calculated Two-Factor Factorial RCBD Report", 0)
                doc.add_paragraph("Analysis generated through OLS multi-factor model fitting with main effects and simple combinations partitioning.")
                ...
                for g_title, g_cols, table_num in groups:
                    ...
                    # Storage structures for table building
                    calculated_means_for_table = {}  # {LevelName: {col: "val_letters"}}
                    calculated_stats_for_table = {k: {} for k in ["sem_a", "lsd_a", "sem_b", "lsd_b", "cv", "interaction", "grand mean"]}
                    
                    for param in g_cols:
                        # 1. Fit Two-Factor factorial model: response ~ Block + A + B + A:B
