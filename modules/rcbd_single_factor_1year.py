...
ACADEMIC_TEMPLATES_1Y = {
    "temp_1_sig_yield": (
        ...
        "phenotypic plasticity among the evaluated germplasm under these environmental conditions. The superiority of **{top_g}** "
        "suggests an optimized metabolic efficiency, robust cellular development, or enhanced assimilation rate, making it a highly promising "
        "candidate for targeted breeding and selection programs. Conversely, the lower boundary was defined by the lowest-performing "
        "genotypes, which demonstrated significantly depressed performance, thereby validating the wide genetic spectrum captured in this trial."
    ),
    "temp_2_sig_quality": (
        ...
        "standard tissue degradation when not buffered by the superior genetic characteristics of **{top_g}**. This suggests that **{top_g}** "
        "possesses a highly efficient physiological or biochemical system for maintaining post-harvest or structural quality throughout "
        "the growth cycle, rendering it highly suitable for value-added industrial applications."
    ),
    "temp_3_strict_superiority": (
        ...
        "unique and efficient phenotypic expression for this trait. This unique performance indicates that the physiological pathways "
    ...
    "temp_10_progressive_decline_trend": (
        ...
        "{last_gm} by {last_day}. Highly significant genotype differences ({last_p}) were recorded on {last_day}, with genotype **{top_g}** "
    ...
    "temp_11_late_onset_divergence": (
        ...
        "in **{table_label}**), where genotype **{top_g}** ({top_val}^{top_let}) established its statistical lead. This pattern suggests that in the "
...
def set_header_bottom_border(row_or_cells):
    ...
    a raw tuple of python-docx cell objects.
    """
...
def run_summary_mode_processing(uploaded_file):
    try:
        ...
        for idx, val in enumerate(df_raw[0]):
            ...
            if val_str == "sem" or val_str == "sem(±)": idx_sem = idx
            elif val_str == "p-value" or val_str == "p_value": idx_pval = idx
